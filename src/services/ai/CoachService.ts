    ...
    private summarizeToolResult(toolName: string, result: ToolResult, toolsUsed: string[]): CoachResponse {
        ...
        switch (toolName) {
            ...
            case 'get_user_status':
                const s = result.data as { caloriesRemaining: number; proteinRemaining: number; currentStreak: number; todayProgress: number } | undefined;
                if (!s) {
                    return { text: 'Unable to fetch your current status.', toolsUsed };
                }
                return {
            ...
            case 'log_verified_food':
                const l = result.data as { food: string; portion: string; nutrition: { calories: number; protein: number } } | undefined;
                if (!l) {
                    return { text: 'Unable to log food.', toolsUsed };
                }
                return {
            ...
            case 'get_food_details':
                const f = result.data as { name: string; servingSize: string; macros: { calories: number; protein: number; carbs: number; fat: number } } | undefined;
                if (!f) {
                    return { text: 'Unable to fetch food details.', toolsUsed };
                }
                return {
