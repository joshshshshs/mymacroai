      ...
      switch (data.type) {
        case 'weight':
          await this.healthConnect.insertRecords([{
            recordType: 'Weight',
            weight: { value: data.value, unit: 'kilogram' },
            time: now,
          }]);
          break;
        case 'bodyFat':
        ...
        case 'hydration':
          await this.healthConnect.insertRecords([{
            ...
            volume: { value: data.value, unit: 'liter' },
            startTime: now,
            endTime: now,
          }]);
          break;
        default:
          logger.warn(`Unsupported data type for HealthConnect write: ${data.type}`);
      }
    } catch (error) {
      logger.error(`HealthConnect write failed for ${data.type}:`, error);
      throw error;
    }
  }
